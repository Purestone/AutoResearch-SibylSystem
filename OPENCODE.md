...
**强烈建议在 tmux 中运行 opencode**，以支持 Sentinel 看门狗自动恢复。安装：`brew install tmux`(macOS) / `apt install tmux`(Linux)。启动：`tmux new -s sibyl`。
...
`.opencode/agents/*.md` 与 `.opencode/skills/*/SKILL.md` 属于运行时资产，必须和 Python 编排器一起版本管理。
...
- Agent tier 通过 `.opencode/agents/sibyl-{heavy,standard,light}.md` 声明式配置
...
双 MCP 架构（通过项目 `.mcp.json` 文件注册；旧环境也可能仍用 `~/.mcp.json`）：
...
飞书同步 skill: `.opencode/skills/sibyl-lark-sync/SKILL.md`
